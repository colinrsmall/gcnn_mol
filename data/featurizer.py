...
class MoleculeFeaturizer:
    ...
    def _featurize_mol(
        self, smiles: str
    ) -> Optional[Tuple[str, npt.ArrayLike, npt.ArrayLike, npt.ArrayLike, npt.ArrayLike]]:
        """
        ...
        bond_feature_matrix = np.zeros((len(adjacency_matrix), len(adjacency_matrix), 21))
        for atom_1 in range(len(adjacency_matrix)):
            for atom_2 in range(len(adjacency_matrix)):
                bond = mol.GetBondBetweenAtoms(atom_1, atom_2)
                if bond:
                    bond_feature_matrix[atom_1][atom_2][int(bond.GetBondType())] = 1
        bond_feature_matrix = torch.from_numpy(bond_feature_matrix).to(torch.float32)
        ...
        return smiles, adjacency_matrix, bond_feature_matrix, atom_feature_matrix, mol_features

    def featurize_datapoint(self, smiles: Union[str, list[str]], number_of_molecules: int, target: float):

        if number_of_molecules == 1:
            smiles, adjacency_matrix, bond_feature_matrix, atom_feature_matrix, mol_features = self._featurize_mol(
                smiles[0]
            )
            return SingleMolDatapoint(
                smiles, adjacency_matrix, atom_feature_matrix, bond_feature_matrix, mol_features, target
            )
            ...
            for smiles in smiles:
                ...
                bond_feature_matrices.append(bond_feature_matrix)
